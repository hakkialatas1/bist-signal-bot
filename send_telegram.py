...
def build_msg(orders_path: str, title: str, top_n: int = 15) -> str:
    ...
    # Ortak kolonlar (bist/us farklı isimli alloc olabilir)
    weight_col = "target_weight_%"
    alloc_col = "target_alloc_TL" if "target_alloc_TL" in orders.columns else ("target_alloc_USD" if "target_alloc_USD" in orders.columns else None)
    ...
    fresh = int(pd.to_numeric(orders["fresh"].iloc[0], errors="coerce") or 0) if "fresh" in orders.columns else 0
    ...
    # Kill-switch: veri taze değilse sadece bilgi bas
    if fresh == 0:
        return "\n".join(lines).strip()
    ...
    if len(buy):
        ...
        for _, r in buy.iterrows():
            alloc_txt = f" (~{int(r[alloc_col])} {('TL' if alloc_col=='target_alloc_TL' else 'USD')})" if alloc_col else ""
...
def main():
    ...
    # 2) US
    us_msg = build_msg("orders_us.csv", "US (DOW30) SİNYAL", top_n=15)
    send_message(us_msg)
